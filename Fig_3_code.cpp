// sib_clvR.c to run recurrence equations of sib mating with ClvR drive that has fitness effects 14 vii 21
...
void run_case(float Sd_Sd_feq, float sC_sC_feq, float sA_in, float d_fit) {
...
//CC[2] = fitness of ClvR homozygotes \n");
	...
	sigma = 1;
	K = 0;
	allmated[1][0][0][1][0][0] = Sd_Sd_feq;
	allmated[0][1][0][0][1][0] = sC_sC_feq;
	...
	printf("gen  \tscg \tscG \tsCg \tsCG \t \tScg \tScG \tSCg \tSCG \tWbar  \tsA (%0.2f) \n", sA);
	...
	for (; gen < totcycles;) {
		for (j = 1; j <= printcycles; j++) {
			...
			for (i1 = 0; i1 < 2; i1++) for (i2 = 0; i2 < 2; i2++) for (i3 = 0; i3 < 2; i3++) for (i4 = 0; i4 < 2; i4++) for (i5 = 0; i5 < 2; i5++) for (i6 = 0; i6 < 2; i6++) {
				...
				if ((i3 + i6 == 2) && (i2 + i5 == 0)) {   // if all G and no C, we will kill them off
	...
	printf("sA = %1.2f  sigma = %1.2f CC[2] = %1.4f  \n\n\n", sA, sigma, CC[2]);

}

//runs to 20000 cycles
rd_t run_case_equ(float Sd_Sd_feq, float sC_sC_feq, float sA_in, float d_fit) {
	int dummy, dummy1, i, j, k, totcycles, printcycles, a, b, c, d, e, f, i1, i2, i3, i4, i5, i6, gen;
	...
	for (; gen < totcycles;) {
		...
		if (gen == totcycles-1) {
			...
			to_r.wb = Wbar;
			to_r.gen = gen;
			to_r.sib_mated_feq = totals[0][0][0]+ totals[0][0][1]+ totals[0][1][0]+ totals[0][1][1]+
				totals[1][0][0]+ totals[1][0][1]+ totals[1][1][0]+ totals[1][1][1];
			return to_r;
		}
...
//generates the data at equalibrium
void equ_data(FILE * of, float sA) {
	fprintf(of,"Fitness, Wbar, sA, sib_feq\n");

	for (float i = 1; i>0; i-=0.01)
	{
		struct rd data = run_case_equ(0.07, 0.07, sA, i);
	
		fprintf(of, "%.3f,%.3f,%.3f,%.3f\n", i, data.wb, sA,data.sib_mated_feq);
		printf("%.1f%%\n", i * 100);
	}
}
...
{
	//system("dir");
	//run_case(0.01, 0.01, 0.6, 0.7);
	FILE* of = fopen("data2.csv", "w");
	equ_data(of, 0.2);

	fclose(of);

	of = fopen("data3.csv", "w");
	equ_data(of, 0.95);

	fclose(of);

	 of = fopen("data1.csv", "w");
	equ_data(of, 0.5);
...
//
	...
	//run_case(0.01, 0.01, 0.1, 0.95);

	//run_case(0.01, 0.01, 0.5, 0.9);
	return 0;
}
